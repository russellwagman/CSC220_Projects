...
public class Pacman {
	...
	*/
	...
	public void testNeighbors1() {
		maze[4][5].visited = true;
		maze[4][6].visited = true;
		backtrack(maze[3][5]);
	}
	...
	public void solveBFS() {
		...
		Node current = new Node(startX, startY, 'S');
		BFS.offer(current);
		while(BFS.size() != 0) {
			current = BFS.remove();
			ArrayList<Node> x = getNeighbors(current);
			for(int i = 0; i < x.size(); i++) {
				x.get(i).visited = true;
				x.get(i).parent = current;
				BFS.offer(x.get(i));
				if(x.get(i).content == 'G') {
					backtrack(x.get(i));
					break;
				}
			}
